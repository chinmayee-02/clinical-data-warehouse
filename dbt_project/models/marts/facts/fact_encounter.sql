{{ config(materialized='incremental', unique_key='encounter_id', schema='GOLD', tags=['gold','fact']) }}
...
FROM encounters e
LEFT JOIN {{ ref('dim_patient') }}   p  ON e.patient_id  = p.patient_natural_key
LEFT JOIN {{ ref('dim_provider') }}  pv ON e.provider_id = pv.provider_natural_key
LEFT JOIN {{ ref('dim_payer') }}     py ON e.payer_id    = py.payer_natural_key
LEFT JOIN {{ ref('dim_condition') }} c  ON e.reason_code = c.condition_natural_key
